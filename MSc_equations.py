...
# set same directory as that loaded in MSc.py
...
def trend(image, plotting = False):
    ...
    2D detrending of image for spectral peak location finder
...
#%% apply sinc kernel for custom low-pass filtering
...
def longWaveFilter(image, samplerate, plotting = False):
    ...
    Remove these components by dividing original image by image_longwave
    ...
    # optionally change to median filter, cost of longer processing times
...
#%% Calculate angle of short wave components for wind directions
...
def peakShortwave(psd2D, samplerate, plotting = False):
    ...
    between spectral wavelengths of 400 and 5000 meter
    """
    ...
    psd2D_weighted_averaged_clipped = np.where((r<400) | (r>5000), np.min(psd2D_weighted_averaged), psd2D_weighted_averaged)
...
def rotateAndClip(image, rotation, samplerate, plotting = False):
    from scipy import ndimage
    """
    Used to rotate and clipe images along or perpendicular to orienatation of greatest variation
...
def applyCMOD(NRCS, phi, incidence, iterations, samplerate = 50, CMOD5 = False, plotting = False):
    ...
    if CMOD5 = True then it will yield CMOD5 results instead of CMOD5.n results
    """
...
def psd1D(image, samplerate, plotting= False, windowed = False, scaled = False, normalised = False):
    ...
    NFFT = np.shape(FFT1)[1]                        # number of points in original FFT
    delta_n = fs / NFFT                             # calculated bin width
    FFT4 = FFT3 / delta_n                           # final scaling to go to PSD
    ...
    # i.e. Taylor's hypothesis of frozen turbulence
    if normalised == True:
        psd1D /= np.mean(image)
    ...
    # Manually select kolmogorov -5/3 drop off range to print drop
    begin = 0.004 # 0.001 for 6500m etc
    end = 0.01  # 0.01 for 650m for 0.0063 for 1000m etc
...
def sikora1997(windspeed, samplerate, window_length = -9999, smoothing_fact = 2, windowed = True, plotting = False):
    ...
    """
    approximately follows method proposed by Sikora et al., 1997 to estimate peak in 1D PSD and thus Zi
    """
    ...
    smoothed_spectrum = ndimage.gaussian_filter1d(PSD[0:], smoothing_fact)
    ...
    try:
        ...
        # determine Z_i paramater using ratio of 1.5 (ratio can be undone after this script to apply different ration (e.g. 2.0 for rolls or so))
        Z_i = peak_spatial_wavelength / 1.5
        
        # following parameters are outdated and not used
        window_length                        # no used
...
#%% first loop of Young's (algorithm 1)
...
def loop1(windfield):
    ...
    # prepare loop of 15 iterations (10 should be sufficient already)
    iterations = 15
...
def loop2A(windfield, surface_stress, friction_velocity, z_0, Zi):
    ...
    for i in range(iterations):
        if i > 0:
            ...
            # # Panofski et al 1977 eq 5, optionally use different emperical functions
...
def loop2B(windfield, friction_velocity, z_0, Zi, Cdn, PSD, samplerate, inertial_max, idx_ll, dissip_rate,  form = 'cells'):
    ...
    inertial_max            # longest wavelength to consider for inertial subrange (Zi * 1.5)
    inertial_min = 650      # shortest wavelength to consider for inertial subrange
    ...
    return sigma_u, L, B, w_star, w_star_normalised_deviation, corr_fact, H 


#%% Loop 2C using TKE dissipation (algorithm 2C)
...
def tiledWind(NRCS, incidence, longitude, latitude, iterations, size, dissip_rate, slope_multiplier = None, \
    ...
    output:
        all parameters from loop 1, 2A, 2B and 2C (algorithms 1, 2A, 2B and 2C)
        
    note:
        section 0 of this script concerns classification of tiles and has been commented out.
            It can be ignored our used as a skeleton for other classification. 
        Loop 2C (algorithm 2C) has also been commented out to save run time. can be uncommented
    ...
    for i, ax in enumerate(np.flipud(np.fliplr(axes).flat)):
        ...
        if np.shape(item)[0] == M & np.shape(item)[1] == N:
           try:
               ...
               """
               1. determine wind direction angle either through spectral peak
               """
               ...
               # calculate wind direction a.k.a. wind origin
               wind_origin = (azimuthAngle - angle) - 180
               if wind_origin <= 0:
                   wind_origin += 360
               
               # if single wind speed direction is given, use that, if array is given use that (input array needs to be identical size as tile array)
               ...
               # determine peak Zi using method described in Sikora 1997, guassian filter used sigma 2 (smoothing factor is 2)
               # window length is legacy input (no longer used)
               try: 
               ...
               if form != 'cells':
                   Zi_ratio = 2.0 # average ratio of 3.39 for rolls according to wang 2019 "characeristics of marine atmospheric boundary layer"
               else:
               ...
               # calculate temporal PSD S(n) (normalised = True)
               ...
               PSD_multiplier = 1 # default is 1.0 and performs no multiplication
               PSD_original = PSD_original * PSD_multiplier
               
               #account for form (e.g. cells vs rolls)
               sigma_u2, L2, B2, w_star2, w_star2_std, corr_fact2, H = loop2B(windspeed, u_star, z_0, Zi, Cdn, PSD_original, samplerate, inertial_max, idx_ll, dissip_rate, form = form) 
               
               # Loop 2C, currently commented to save processing time
               ...
               # use original PSD (preferred) or smoothed spectrum for plotting normalised with wavelength (smoothed_spectrum from sikora)
               ...
               # if threshold is not set, ignore by setting to infinity
    ...
    """
    # delete anomalous high S(n) values (peak value twice median peak value)
    ...
    for i, ax in enumerate(np.flipud(np.fliplr(axes).flat)):
        ...
        # zero values mean no calculation was performed, set to nan, (alternatively fill initial list with nan's instead of zeros)
        ...
        if np.shape(item)[0] == M & np.shape(item)[1] == N:
           try:
               ...
               if np.isnan(PSD_plot).any():
                   ...
                   ax.set_ylim(kolmogorov[-1] / 5, kolmogorov[0] * 50)
